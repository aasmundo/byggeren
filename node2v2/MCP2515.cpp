...
#define RXB1SIDL (RXB1SIDH + 1) 
#define RXB1EID8 (RXB1SIDH + 2)
#define RXB1EID0 (RXB1SIDH + 3)
#define RXB1DLC (RXB1SIDH + 4)
#define RXB1D0 (RXB1SIDH + 5)
...
boolean MCP2515::transmitCANMessage(CANMSG msg, unsigned long timeout)
{
  ...
  {
    val = readReg(CANINTF);
    if(bitRead(val,TX0IF) == 1 || bitRead(val, 2) == 1 || bitRead(val, 4) == 1)
    {
      sentMessage = true;
      break;
    }
